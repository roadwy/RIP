...
import (
	...
	"errors"
	"math"
	"teamserver/pkg/crypto/chacha20/internal/api"
	"teamserver/pkg/crypto/chacha20/internal/hardware"
	"teamserver/pkg/crypto/chacha20/internal/ref"
)
...
func Chacha20Crypt(src, key []byte) (dst []byte, err error) {
	if len(key) != KeySize+NonceSize {
		return dst, errors.New("session key length error")
	}
	enc, err := New(key[:KeySize], key[KeySize:])
	if err != nil {
		return
	}
